...
function updateCarUI(){if(!selectedCar)return;const range=getRange(selectedCar);const estimated=!selectedCar.consumption&&selectedCar.battery&&range;const consumption=selectedCar.consumption||(estimated?selectedCar.battery/range*100:null);selectedCar._calcConsumption=consumption;const cp=chargingProfile(selectedCar);const rangeText=range?num(range)+' km':'Não informado';const source=selectedCar.rangeSource==='INMETRO'?'INMETRO/PBEV 2026': 'INMETRO não localizado — não usado no cálculo';const specs=[['Propulsão',typeName(selectedCar.type)],['Bateria',selectedCar.battery?num(selectedCar.battery)+' kWh':'—'],['Autonomia INMETRO',rangeText],['Recarga',cp.label||((cp.dc?'AC + DC':'AC'))],['Consumo',consumption?num(consumption)+' kWh/100 km':'—'],['Potência',selectedCar.power||'—']];$('carSpecs').innerHTML=specs.map(x=>`<div class="spec"><span>${x[0]}</span><b>${safe(x[1])}</b></div>`).join('');$('dataQuality').innerHTML=`<span class="${selectedCar.rangeSource==='INMETRO'?'data-ok':'data-warn'}">${safe(source)}</span>${selectedCar.catalogRange&&!range?` · valor de catálogo ${num(selectedCar.catalogRange)} km foi descartado.`:''}`;updateFuelBox();updateChargingUI();}
